...
app.patch(
  ...
  ruta(async (req, res) => {
    ...
    if (patron.length < 2)
      return res.status(400).json({ error: "El patrón es demasiado corto." });
    ...
    try {
      ...
      if (e.code === "ER_DUP_ENTRY")
        return res
          .status(409)
          .json({ error: "Ya existe una regla con ese patrón." });
      throw e;
    }
    res.json({ ok: true });
  }),
...
app.get(
  ...
  ruta(async (req, res) => {
    ...
    SELECT MONTH(m.fecha) mes,
     ...
     GROUP BY MONTH(m.fecha) ORDER BY mes`,
      [anio],
    );
    ...
    SELECT COUNT(*) n, ROUND(SUM(ABS(importe)), 2) importe
      FROM movimientos WHERE YEAR(fecha) = ? AND estado = 'pendiente'`,
      [anio],
    );

    const anios = (
      await pool.query(
        "SELECT DISTINCT YEAR(fecha) a FROM movimientos ORDER BY a DESC",
      )
    )[0].map((r) => r.a);

    // mysql2 devuelve los DECIMAL como texto (decimalNumbers: false, para no
    // perder céntimos por redondeo de floats). Aquí sí interesa sumarlos con
    // JS, así que se convierten explícitamente en vez de dejar que las
    // columnas numéricas viajen como string y el "+" del cliente las concatene
    // en lugar de sumarlas.
    const numerizar = (filas, campos) =>
      filas.map((f) => {
        const copia = { ...f };
        campos.forEach((c) => {
          if (copia[c] != null) copia[c] = Number(copia[c]);
        });
        return copia;
      });
    ...
    res.json({
      anio,
      anios,
      porMes: numerizar(porMes, ["ingresos", "gastos", "ahorro"]),
      porCategoria: numerizar(porCategoria, ["importe"]),
      reparto: numerizar(reparto, ["importe"]),
      comparativa: numerizar(comparativa, ["ingresos", "gastos"]),
      pendiente: { n: Number(pend.n), importe: Number(pend.importe || 0) },
    });
  }),
);
...
app.listen(puerto, host, () =>
  console.log(`Presupuesto en http://${host}:${puerto}`),
);
